...
* **Persistent Login:** Users remain logged in across application restarts using MSAL's secure token cache.
...
* **Personalized & Private Data:**
    ...
    * A robust "Wipe Data" feature with a confirmation prompt allows users to securely delete all of their content from the database.
...
The application is built using modern, best-practice software patterns to ensure stability, maintainability, and a responsive user experience.

* **Asynchronous-First Approach:** The entire application, from startup to data fetching, is fully asynchronous using `async`/`await`. This prevents blocking the main UI thread, eliminating freezes and "Application Not Responding" (ANR) errors.
* **Decoupled Modal Login Flow:** The app starts on a dedicated `LoginPage` completely separate from the main `AppShell`. The `AppShell` is only instantiated and displayed *after* a successful authentication, avoiding complex startup navigation race conditions.
* **Orchestrated Initialization:** A dedicated `InitializationService` runs all necessary background tasks (like data cleanup) after a user signs in but *before* the main UI is shown, guaranteeing the app is in a fully ready state.
* **MVVM Design Pattern:** The app strictly follows the Model-View-ViewModel (MVVM) pattern, leveraging the **Community Toolkit MVVM** for source-generated properties and commands. This creates a clean separation between the UI (XAML Views) and the application logic (ViewModels).
* **Decoupled ViewModel Communication:** ViewModels and services communicate using the `WeakReferenceMessenger`, preventing tight coupling and memory leaks.
* **Dependency Injection (DI):** All services and ViewModels are managed by .NET MAUI's built-in DI container, promoting modular and testable code. Circular dependencies are resolved using the `IServiceProvider` pattern for lazy resolution.
## Technology Stack
...
* **Authentication:** Microsoft Entra ID (using the MSAL.NET library)
* **Primary Language:** C#
...
* **`TrackCubed.Shared/`**: A class library containing shared models (database entities) and DTOs used by both the API and MAUI apps.
...
2.  Ensure Android-specific configurations (`AndroidManifest.xml` and `MainActivity.cs`) are in place.
...
* [x] **Architect a robust, non-blocking startup and authentication flow**
